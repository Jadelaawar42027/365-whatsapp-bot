...
export async function upsertContactMemory(caller, contactId, requestedBrokerId, fields) {
  ...
  // reassign a contact that already belongs to a different broker.
  const existing = await pool.query(
    "SELECT broker_id FROM contacts_memory WHERE contact_id = $1",
    [contactId]
  );
  ...
  const { rows } = await pool.query(
    `INSERT INTO contacts_memory
     ...
     ON CONFLICT (contact_id) DO UPDATE SET
       ...
       consecutive_missed_followups = COALESCE(EXCLUDED.consecutive_missed_followups, contacts_memory.consecutive_missed_followups),
       ...
       stale = COALESCE(EXCLUDED.stale, contacts_memory.stale),
       updated_at = now()
     RETURNING *`,
    [contactId, brokerId, consecutiveMissedFollowups, lastAiSummary, sentimentTrend, lastContactedAt, confidence, stale]
  );
  return rows[0];
}
